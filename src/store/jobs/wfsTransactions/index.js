...
const namespace = 'WFSTRANSACTIONS_JOBS';
...
export function fetch(organizationId, accountId, data) {
  return (dispatch) => {
    ...
    return watchSlice(`state/jobs/wfsTransactions/${organizationId}/${accountId}`, { parameter: 'status', parameterValue: data.status, limitToLast: 1000 }, (_data) => {
...
export function clear() {
  return (dispatch, getState) => {
    removeListeners(getState().jobs.wfsTransactions.data.paths);

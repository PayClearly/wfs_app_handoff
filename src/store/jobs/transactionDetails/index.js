...
const namespace = 'TRANSACTION_JOBS';
...
export function fetch(organizationId, accountId, data) {
  return (dispatch) => {
    ...
    return watchSlice(`state/jobs/transactionDetails/${organizationId}/${accountId}`, { parameter: 'status', parameterValue: data.status, limitToLast: 1000 }, (_data) => {
...
export function clear() {
  return (dispatch, getState) => {
    removeListeners(getState().jobs.transactionDetails.data.paths);

...
const namespace = 'CHECKSINTEGRATION';
...
export function _moduleReducers(state = defaultState, action) {
  switch (action.type) {
    case actionTypes.fetchStart:
      if (action.items && action.items.checkProof) {
        return {
          ...state,
          checkProof: action.items.checkProof,
        };
      }
      return {
        ...state,
...
export function sync(organizationId, accountId) {
  const integration = 'checksIntegration';
...
export function syncRelevantData(organizationId, accountId, paymentId) {
  return (dispatch, getState) => {
    ...
    const checksInStore = getState().account.checksIntegration.data.resources.checks;

    dispatch({ type: actionTypes.fetchStart });

    // Payments have _oldId as legacy id or duplicate of new push Ids
    if (checksInStore[payment._oldId]) return dispatch({ type: actionTypes.fetchSuccess });

    const resources = ['checks'];

    return Promise.all(
      resources.map((resource) => {
        return watchValue(`state/links/checksIntegration/${organizationId}/${accountId}/resources/${resource}/${payment._oldId}`, (item, paths) => { 
...
export function updatePreferences(organizationId, accountId, data) {
  return (dispatch, getState) => {
    ...
    return API_checksIntegration.updatePreferences(organizationId, accountId, data)
    .then((res) => {
...
export function update(organizationId, accountId, data) {
  return (dispatch, getState) => {
    dispatch({ type: actionTypes.updateStart });
    return API_checksIntegration.update(organizationId, accountId, data)
    .then((res) => {
      dispatch({ type: actionTypes.updateSuccess, data: res });
    })
    .catch((error) => {
...
export function link(organizationId, accountId, data) {
  return (dispatch, getState) => {
    ...
    return API_checksIntegration.link(organizationId, accountId, data)
    .then((res) => {
...
export function unlink(organizationId, accountId, data) {
  return (dispatch, getState) => {
    dispatch({ type: actionTypes.updateStart });
    return API_checksIntegration.unlink(organizationId, accountId, data)
    .then((res) => {
      dispatch({ type: actionTypes.updateSuccess, data: res });
    })
    .catch((error) => {
...
export function getIssues(organizationId, accountId, issueType) {
  return (dispatch, getState) => {
    dispatch({ type: actionTypes.fetchStart });
    const ids = getState().account.checksIntegration.data.metas || {};
    
    let data = {};
    return Promise.all(Object.keys(ids || {}).map((key) => {
      return watchValues(`state/links/checksIntegration/${organizationId}/${accountId}/links/${key}`, Object.keys(ids[key][issueType.toLowerCase()] || {}), (items, paths) => {
        data = { ...items, ...data };
...
export function syncQueue(queue) {
  return (dispatch, getState) => {
    ...
    return API_integrationManagement.sync(organizationId, accountId, 'checksIntegration', queue)
    .then((res) => {
...
export function fetchCheckProofImage() {
  return (dispatch, getState) => {
    dispatch({ type: actionTypes.fetchStart, items: { checkProof: { status: 'fetching' } } });
    const organizationId = getState().organization.data.id;
    const accountId = getState().account.data.id;
    return API_checksIntegration.getProviderSetupResource(organizationId, accountId, 'getCheckProofReviewImage')
    .then((res) => {
      if (res.data && res.data.error) {
        dispatch({ type: actionTypes.fetchSuccess, items: { checkProof: { status: 'error', error: res.data.error } } });
      }
      dispatch({ type: actionTypes.fetchSuccess, items: { checkProof: { status: 'fetched', checkProofId: res.data.data.checkProofId, image: res.data.data.image } } });
    });
  };
}
...
export function clear() {
  return (dispatch, getState) => {
    removeListeners(getState().account.checksIntegration.data.paths);

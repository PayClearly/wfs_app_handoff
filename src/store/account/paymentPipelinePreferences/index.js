...
const namespace = 'ACCOUNTPAYMENTPIPELINEPREFERENCES';
...
export function update(orgId, accountId, data) {
  return (dispatch) => {
    ...
    return AccountsAPI.updatePaymentPipelinePreferences(orgId, accountId, { ...data })
    .then(() => {
...
export function sync(organizationId, accountId) {
  return (dispatch) => {
    ...
    return watchValue(`state/paymentPipelinePreferences/${organizationId}/${accountId}`, (item, paths) => {
...
export function clear() {
  return (dispatch, getState) => {
    removeListeners(getState().account.paymentPipelinePreferences.data.paths);

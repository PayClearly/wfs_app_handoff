...
const namespace = 'ACCOUNTPAYMENTCARDCUSTOMFIELDS';
...
export function update(orgId, accountId, data) {
  return (dispatch) => {
    ...
    return AccountsAPI.updatePaymentCardCustomFields(orgId, accountId, { ...data })
    .then(() => {
...
export function sync(organizationId, accountId) {
  return (dispatch) => {
    ...
    return watchValue(`state/paymentCardCustomFields/${organizationId}/${accountId}`, (item, paths) => {
...
export function clear() {
  return (dispatch, getState) => {
    removeListeners(getState().account.paymentCardCustomFields.data.paths);

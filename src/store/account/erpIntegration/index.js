...
import API_erpIntegration from 'api/erpIntegration';
import API_integrationManagement from 'api/integrationManagement';

const namespace = 'ERP_INTEGRATION';
...
export function updatePreferences(organizationId, accountId, data) {
  return (dispatch, getState) => {
    ...
    return API_erpIntegration.updatePreferences(organizationId, accountId, data)
    .then((res) => {
...
export function update(organizationId, accountId, data) {
  return (dispatch, getState) => {
    dispatch({ type: actionTypes.updateStart });
    return API_erpIntegration.update(organizationId, accountId, data)
    .then((res) => {
      dispatch({ type: actionTypes.updateSuccess, data: res });
    })
    .catch((error) => {
...
export function link(organizationId, accountId, data) {
  return (dispatch, getState) => {
    ...
    return API_erpIntegration.link(organizationId, accountId, data)
    .then((res) => {
...
export function unlink(organizationId, accountId, data) {
  return (dispatch, getState) => {
    dispatch({ type: actionTypes.updateStart });
    return API_erpIntegration.unlink(organizationId, accountId, data)
    .then((res) => {
      dispatch({ type: actionTypes.updateSuccess, data: res });
    })
    .catch((error) => {
...
export function createVendor(organizationId, accountId, data) {
  return (dispatch, getState) => {
    dispatch({ type: actionTypes.updateStart });
    return API_erpIntegration.createVendor(organizationId, accountId, data)
    .then((res) => {
      dispatch({ type: actionTypes.updateSuccess, data: res });
    })
    .catch((error) => {
...
export function getIssues(organizationId, accountId, issueType) {
  return (dispatch, getState) => {
    ...
    return Promise.all(Object.keys(ids || {}).map((key) => {
      return watchValues(`state/links/erpIntegration/${organizationId}/${accountId}/links/${key}`, Object.keys(ids[key][issueType.toLowerCase()] || {}), (items, paths) => {
        data = { ...items, ...data };
...
export function syncQueue(queue) {
  return (dispatch, getState) => {
    ...
    return API_integrationManagement.sync(organizationId, accountId, 'erpIntegration', queue)
    .then((res) => {

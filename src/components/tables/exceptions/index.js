...
class components_tables_csrpayments extends Component {
  ...
  refRenderer = (paymentStatus) => {
    ...
    const path = `${router.baseUrl}/csrpayment/${paymentStatus._id}/?orgId=${orgId}&accountId=${accountId}`;
  ...
  rowRenderer = (rowId, rowData, expanded) => (
    <div className="py-2">
      <div className="card mb-2">
        ...
        <div className="px-3">
          <Components.overviews.csrPaymentStatus id={rowId} />
        </div>
      </div>
      <div className="card">
        <h4 className="card-title mt-3 ms-3">User View</h4>
        <div>
          <Components.overviews.paymentstatus.view id={rowId} />
        </div>
      </div>
    </div>
  );

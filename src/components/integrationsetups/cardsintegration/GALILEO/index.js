...
const mapDispatchToProps = (dispatch) => {
  return ({
    link: () => {
      dispatch(Store.account.linkIntegration('cardsIntegration', { provider: 'GALILEO' }));
    },
  });
};
...
class components_integrationsetups_cardsintegration_GALILEO extends Component {
  ...
  render() {
    ...
    if (!_try(() => this.props.cardsIntegration.linked)) {
      renderBody = (
        <Fragment>
          <p>By setting up an account with Galileo you will be able to pay vendors using virtual cards. If you are unsure what this means or would like more information please contact {this.props.providerTheme.displayName} Support at {this.props.providerTheme.supportEmail}</p>
          <a
            tabIndex="-1"
            role="button"
...
export default connect(mapStateToProps, mapDispatchToProps, mapResourcesToProps)(components_integrationsetups_cardsintegration_GALILEO);

...
const mapDispatchToProps = (dispatch, props) => {
  return ({
    create: (data) => {
      dispatch(Store.account.createClient(data));
    },
...
class components_creators_client extends Component {
  ...
  render() {
    ...
    return (
      <Components.creators.creatorwrapper
        className="components_creators_client"
        canCreate={canCreate}
        createFormActive
        status={status}
        includeButton={!modal}
        onCreateNotification="Client successfully created!"
        createDisabled={!form._allValid || status.creating}
...
export default connect(mapStateToProps, mapDispatchToProps, mapResourcesToProps)(components_creators_client);

...
const watchedEqualityCheck = (previousVal, currentVal) => {
  ...
  if (!equal && window.GLOBALCERT.projectId === 'payclearly-test') console.log(previousVal, 'was not equal to current value', currentVal);
  return equal;
};
...
function selectorWrapper(...args) {
  ...
  const _wrapped = (...params) => {
    ...
    if (took > 1000 && window.GLOBALCERT.projectId === 'payclearly-test') console.log(`--- (${name || 'unnamed'}) took ${took}ms`);
